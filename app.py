...
if len(h_list) >= 5:
    # 修正ポイント：変数名から記号を削除しました
    m_1 = st.sidebar.selectbox("◎ 本命", h_list, index=0)
    m_2 = st.sidebar.selectbox("○ 対抗", h_list, index=1)
    m_3 = st.sidebar.selectbox("▲ 黒三角", h_list, index=2)
    m_4 = st.sidebar.selectbox("△ 白三角", h_list, index=3)
    m_5 = st.sidebar.selectbox("× ペケ", h_list, index=4)
    marks = {'◎': m_1, '○': m_2, '▲': m_3, '△': m_4, '×': m_5}
